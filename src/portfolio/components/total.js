...
const PortfolioTotalValueChart = ({ stocks }) => {
  ...
  useEffect(() => {
    const stockRefs = stocks.map(stock => ref(database, `live-crypto-data/${stock.ticker}`));
        ...
        onValue(stockRef, (snapshot) => {
          ...
          if (data) {
            ...
            processedData.forEach((entry) => {
              ...
              if (date > tenMinutesAgo) {  // Only include data from the last 10 minutes
                if (!combinedData[date]) {
                  combinedData[date] = { date: date.toLocaleString(), totalValue: 0 };
                }
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={chartData}>
          <CartesianGrid strokeDasharray="3 3" stroke="#333" /> {/* Darker grid to fit dark mode */}
          <XAxis dataKey="date" stroke="#ffffff" /> {/* White X-axis for visibility */}
          <YAxis 
            scale="log" 
          ...
          />
          <Tooltip 
            contentStyle={{ backgroundColor: '#1e1e1e', borderColor: '#8884d8' }} 
            formatter={(value) => `$${value.toFixed(2)}`}  // Format tooltip values
          />
          <Line type="monotone" dataKey="totalValue" stroke="#ff9800" strokeWidth={2} dot={false} /> {/* Orange line with no dots */}
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
